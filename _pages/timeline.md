...
.tl-step.tl-drag-above{background:#f4edf8;margin-top:24px}
.tl-step.tl-drag-above::before{content:"Drop here";position:absolute;top:-20px;left:0;right:0;height:4px;background:#56018D;border-radius:2px;font-size:10px;color:#56018D;font-weight:700;line-height:0;padding-left:8px;text-indent:0}
.tl-step.tl-drag-below{background:#f4edf8;margin-bottom:24px}
.tl-step.tl-drag-below::after{content:"Drop here";position:absolute;bottom:-20px;left:0;right:0;height:4px;background:#56018D;border-radius:2px;font-size:10px;color:#56018D;font-weight:700;line-height:0;padding-left:8px;text-indent:0}
...
function renderControls(){
  ...
  for(var i=0;i<steps.length;i++){
    ...
    h+='<div class="tl-step'+(hidden?" hidden-mode":"")+'" draggable="true" data-idx="'+i+'">';
    h+='<span class="tl-drag" title="Drag to reorder">&#x2847;</span>';
...
function initDrag(){
  ...
  items.forEach(function(item){
    item.addEventListener("dragstart",function(e){
      dragIdx=+item.getAttribute("data-idx");
    ...
    item.addEventListener("dragover",function(e){
      ...
      if(e.clientY<rect.top+rect.height/2) item.classList.add("tl-drag-above");
      else item.classList.add("tl-drag-below");
    });
    item.addEventListener("dragleave",function(){
      item.classList.remove("tl-drag-above","tl-drag-below");
    });
    item.addEventListener("drop",function(e){
      e.preventDefault();
      var targetIdx=+item.getAttribute("data-idx");
      if(dragIdx===null||dragIdx===targetIdx){clearDragClasses();return;}
      var rect=item.getBoundingClientRect();
      var insertIdx=e.clientY<rect.top+rect.height/2?targetIdx:targetIdx+1;
...
function clearDragClasses(){
  document.querySelectorAll(".tl-drag-above,.tl-drag-below").forEach(function(el){el.classList.remove("tl-drag-above","tl-drag-below");});
}
...
window.updVal=function(i,v){steps[i].value=v;renderControls();drawChart();};
